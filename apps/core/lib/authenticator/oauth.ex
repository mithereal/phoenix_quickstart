defmodule Core.Authenticator.OAuth do
  import Ecto.Query
  alias DB.Repo
  alias DB.Schema.User
  alias Core.Account.Account
  alias Core.Authenticator.OAuth.Facebook
  alias Core.Authenticator.ProviderInfos
  alias Kaur.Result
  require Logger
    ...
    case find_existing_user(infos) do
      nil ->
        ...
        case infos.provider do
          ...
          :google ->
            user = %User{google_user_id: nil}
            # A user for this account already exists, link its gid
            link_provider!(user, infos)
        end
  ...
  @doc """
  In case we have two accounts for this google / email we match on google first
  """
  def find_existing_user(%{provider: :google, uid: google_user_id, email: email}) do
    User
    |> where([u], u.google_user_id == ^google_user_id)
    |> or_where([u], u.email == ^(email || ""))
    |> Repo.all()
    |> Enum.reduce(
      nil,
      fn user, best_fit ->
        if user.google_user_id == google_user_id or is_nil(best_fit),
          do: user,
          else: best_fit
      end
    )
  ...
  def link_provider!(user, provider_infos) do
    ...
    {:ok, updated_user} =
      ...
      |> Repo.update!()
      |> Account.unlock_achievement(:social_networks)

    # Email is always verified by third party, so it's save to confirm it
    Account.confirm_email!(updated_user)

    # Try to fetch picture
    picture_url = provider_infos.picture_url

    case picture_url && Account.fetch_picture(updated_user, picture_url) do
      {:ok, final_user} ->
        final_user
  ...
  defp provider_info_to_user_info(infos = %{provider: :google}) do
    %{
      google_user_id: infos.uid,
      name: infos.name,
      email: infos.email,
      password: NotQwerty123.RandomPassword.gen_password()
    }
